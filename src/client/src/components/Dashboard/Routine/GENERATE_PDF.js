      ...
      },
      {
        ...
        columns: [
          ...
          },
          {
            ...
            table: {
              headerRows: 1,
              widths: ['*', '*', '*'], // Generating column header width, how much space a column would take
  ...
  let routineCellHeader = routineDB
    ? routineDB.map((singlePeriodObj, index) => {
        if (singlePeriodObj.break.isBreak) {
          return {
            margin: [0, 100, 0, 0], // [Left, Top, Right, Bottom]
            rowSpan: 7,
            text: {
              alignment: 'center',
              text: `${singlePeriodObj.period.split('-')[0]}\n to \n ${
                singlePeriodObj.period.split('-')[1]
              } ${singlePeriodObj.break.msg
                .split(' ')
                .filter(e => e)
                .join('\n')}`,
              bold: true
          ...
          return { text: singlePeriodObj.period, bold: true };
        }
      })
    : [];
  if (routineDB) {
    // console.log('Table Header =>', [alwaysAlwaysHave, ...routineCellHeader]);
    return [{ text: alwaysAlwaysHave, bold: true }, ...routineCellHeader];
  }
...
const generateTableBodyRow = (routineDB, cellDayName, dayName) => {
  let arrayOfCellData = [{ text: cellDayName, bold: true }];
  if (routineDB) {
